...
export default async function AdminProdi() {
    ...
    <div className="flex h-screen bg-gray-50">
      <aside className="w-64 bg-slate-900 text-white shadow-xl flex flex-col hidden md:flex">
        ...
        <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
          ...
          <Link href="/admin/prodi" className="block p-3 rounded-lg bg-slate-800 text-white font-medium text-sm">Program Studi</Link>
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        <header className="h-16 bg-white border-b flex items-center justify-between px-6 z-10">
          <h2 className="text-xl font-semibold text-gray-800">Manajemen Program Studi</h2>
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full text-left text-sm whitespace-nowrap">
                ...
                <tbody className="divide-y divide-gray-100">
                  {prodis.length === 0 ? (
                    <tr><td colSpan={6} className="px-6 py-4 text-center text-gray-500">Belum ada data program studi.</td></tr>
                  ) : (
                    prodis.map((prodi: any) => (
                      <tr key={prodi.id.toString()} className="hover:bg-gray-50">
                        <td className="px-6 py-4 font-medium text-gray-900">{prodi.namaProdi}</td>
                        <td className="px-6 py-4 text-gray-700">{prodi.fakultas}</td>
                        <td className="px-6 py-4 text-gray-700">{prodi.jenjang}</td>
                        <td className="px-6 py-4 text-gray-700">{prodi.akreditasi}</td>
                        <td className="px-6 py-4">
                           <span className={`px-2 py-1 rounded-full text-xs font-semibold ${prodi.status ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                             {prodi.status ? 'Aktif' : 'Non-Aktif'}
                           </span>
                        </td>
                        <td className="px-6 py-4">
